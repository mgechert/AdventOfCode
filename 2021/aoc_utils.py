...
class PriorityQueue:
    """Generalized priority queue originally from AOC 2021 day 15, see also https://docs.python.org/3/library/heapq.html"""
    def __init__(self):
        self.pq = []
        self.entry_finder = {}
    ...
    def remove_item(self, item):
        """Mark a queue entry as removed by removing it from the entry_finder and setting the item to None in the pq"""
    ...
    def update(self, item, priority):
        """
        If specified priority is _higher_ than the item's current priority, no changes are made.
        If item exists in the queue and the specified priority is _lower_ than its current priority, update the item's
        queue priority to the lower priority.
